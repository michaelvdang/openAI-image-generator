  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        alert('CREATEPOST ERROR: ' + error)
        if (error.response) {
          console.log(error.response.status);
          console.log(error.response.data);
        } else {
          console.log('From CreatePost: ' + error.message);
        }
      } finally {
  ...
  return (
    <section className='max-w-7xl mx-auto'>
      <div>
        ...
        <p className='mt-2 text-[#666e75] text-[14[x] max-w-[500px]'>Create imaginative 
      ...
      <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
        ...
        <div className='mt-10'>
          ...
          >Once you have created the image you want, you can share it with others on the community</p>
          <button 
            type='submit'
